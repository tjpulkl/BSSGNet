...
from dataset.datasets import LIPDataValSet, LIPDataSet
import os
...
def valid(model, valloader, input_size, num_samples, gpus):
    ...
    # eval_scale=[0.75,1.0,1.25,1.5,1.75]
    # eval_scale=[1.0]
...
def main():
    ...
    model = Res_Deeplab(num_classes=args.num_classes, img_width = w, img_height=h)
    ...
    best_accuracy = 0
    best_epoch=141
    for nfile in range( 149, 150 ):
        # train_pth = os.path.join( restore_from, "LIP_epoch_149[CIHP67_41]"+".pth")
        # train_pth = os.path.join( restore_from, "LIP_epoch_"+str(nfile)+".pth")
        train_pth = os.path.join( restore_from, "LIP_epoch_"+"Best.pth")
        print(train_pth)
        state_dict = model.state_dict().copy()
        # print( state_dict.keys() )
        state_dict_old = torch.load(train_pth)
        # print( state_dict_old.keys() )
        ...
        print(num_samples/(end-start))
        mIoU = compute_mean_ioU(parsing_preds, scales, centers, args.num_classes, args.data_dir, input_size)
        print(mIoU)
        #=================================================================only used for output result
        # list_path = os.path.join('/mnt/data/humanparsing/CIHP', args.dataset + '_id.txt')
