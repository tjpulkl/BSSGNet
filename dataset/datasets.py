...
class LIPDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.dataset != 'train':
            ...
            hgt, wgt = generate_hw_gt(label_parsing,20)                   
            label_parsing = torch.from_numpy(label_parsing)           
            # label_edge = torch.from_numpy(label_edge) 

            return input, label_parsing, hgt, wgt, meta
...
class LIPDataValSet(data.Dataset):
    def __init__(self, root, dataset='val', crop_size=[384, 384], transform=None, flip=False):
        ...
        list_path = os.path.join(self.root, self.dataset + '_id.txt')
        # list_path = os.path.join('./dataset', self.dataset + '_id.txt')     #only test for output parsing result
        print( list_path )
