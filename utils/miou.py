...
LABELS3 = ['Background', 'face','sunglass','hat','scarf','hair','upper-clothes','left-arm'\
          'right-arm','belt','pants','left-leg','right-leg','skirt','left-shoes','right-shoes','bag','dress']

ADE20K =['wall','building','sky','floor','tree','ceiling','road','bed','window','grass','cabinet','sidewalk','person',\
        'ground','door','table','mountain','plant','curtain','chair','car','water','painting','sofa','shelf','house','sea',\
        'mirror','rug','field','armchair','seat','fence','desk','rock','closet','lamp','bathtub','rail','cushion','pedestal'\
        'box','column','signboard','dresser','counter','sand','sink','skyscaper','fireplace','icebox','grandstand','path',\
        'stairs','runway','showcase','pooltable','pillow','screen','staircase','river','bridge','bookcase','blind','coffeetable',\
        'toilet','flower','book','hill','bench','countertop','stove','palm','kitchen island','computer','swivel chair','boad',\
        'bar','arcade machine','hovel','bus','towel','light','truch','tower','pendant','sunshade','streetlight','booth','TV',\
        'airplane','dirt track','apparel','pole','soil','bannister','escalator','ottoman','bottle','buffet','poster','stage',\
        'van','ship','fountain','conveyer','canopy','washer','toy','swimmingpool','stool','barrel','basket','waterfall','tent','bag',\
        'motorbike','cradle','oven','ball','food','step','tank','brand','microwave','pot','animal','bicycle','lake','dishwasher','silverscreen',\
        'blanket','sculpture','hood','sconce','vase','stoplight','tray','ashcan','fan','dock','crtscreen','plate','monitor','noticeboard',\
...
def compute_mean_ioU_ADE20K(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='val'):
    list_path = os.path.join(datadir, dataset + '_id.txt')
    val_id = [i_id.strip() for i_id in open(list_path)]
    ...
    for i, (label, iou) in enumerate(zip(ADE20K, IoU_array)):
        name_value.append((label, iou))
...
def compute_mean_ioU_ATR(preds, scales, centers, num_classes, datadir, input_size=[473, 473], dataset='val'):
    list_path = os.path.join(datadir, dataset + '_id.txt')
    ...
    tpFg = tp[1:]
    posFg = pos[1:]
    pixel_accuracy_fg = (tpFg.sum() / posFg.sum()) * 100
    precision = ( tp / res )
    recall = ( tp / pos )
    ...
    # for i, (label, iou) in enumerate(zip(LABELS3, IoU_array)):
    #     name_value.append((label, iou))    
    name_value.append(('Pixel accuracy', pixel_accuracy))
    name_value.append(('Pixel_accuracy_fg', pixel_accuracy_fg))
    name_value.append(('Precision', precision))
    name_value.append(('Recall', recall))
    name_value.append(('F1_score', F1_score))
    name_value.append(('Mean accuracy', mean_accuracy))
    name_value.append(('Mean IU', mean_IoU))
    name_value = OrderedDict(name_value)
    return name_value
